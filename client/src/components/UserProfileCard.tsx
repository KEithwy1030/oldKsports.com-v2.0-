...
import { Calendar, Trophy, Briefcase } from 'lucide-react';
    ...
    <div className={`bg-white/10 backdrop-blur-sm rounded-lg border border-white/20 p-6 ${className}`}>
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {!isOwnProfile && (
            <p className="text-gray-300 text-sm mb-3">{currentUser.email}</p>
          )}
          ...
          <div className="flex items-center space-x-4 text-xs text-gray-400">
            <div className="flex items-center space-x-1">
              ...
              <span>加入：{new Date(currentUser.joinDate).toLocaleDateString('zh-CN')}</span>
            </div>
            <div className="flex items-center space-x-1">
